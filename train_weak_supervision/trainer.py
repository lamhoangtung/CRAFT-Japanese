...
from src.utils.utils import (calculate_batch_fscore, calculate_fscore,
                             resize_bbox)
from train_weak_supervision.dataloader import (DataLoaderEvalOther,
                                               DataLoaderEvalOther_Datapile,
                                               DataLoaderMIX,
                                               DataLoaderMIX_JPN)
...
def save(no, dataset_name, output, image, character_map, affinity_map, character_weight, affinity_weight):
    ...
    for __, _ in enumerate(dataset_name):
        ...
        plt.imsave(
            ...
            '/char_map.png', output[__, 0].data.cpu().numpy(),
            cmap='gray')

        plt.imsave(
            'Temporary/'+str(no)+'/'+str(__) +
            '/aff_map.png', output[__, 1].data.cpu().numpy(),
            cmap='gray')

        plt.imsave(
            'Temporary/' +
            str(no)+'/'+str(__) +
            '/target_char_map.png', character_map[__].data.cpu().numpy(),
            cmap='gray')

        plt.imsave(
            'Temporary/' +
            str(no)+'/'+str(__) +
            '/target_affinity_map.png', affinity_map[__].data.cpu().numpy(),
            cmap='gray')

        plt.imsave(
            'Temporary/' +
            str(no)+'/'+str(__) +
            '/weight_char_map.png', character_weight[__].data.cpu().numpy(),
            cmap='gray')

        plt.imsave(
            'Temporary/' +
            str(no)+'/'+str(__) +
            '/weight_affinity_map.png', affinity_weight[__].data.cpu().numpy(),
            cmap='gray')
...
def train(model, optimizer, iteration):
    ...
    for no, (
        ...
        if (no + 1) % config.check_iterations == 0:
            ...
            save(no, dataset_name, output, image, character_map,
                 affinity_map, character_weight, affinity_weight)
        ...
        if (no + 1) % config.calc_f_score == 0:
            ...
            for __, _ in enumerate(dataset_name):
                ...
                if _ != 'SYNTH':
                    ...
                    predicted_ic13.append(resize_bbox(
                        original_dim[__], output[__], config)['word_bbox'])
                    target_ic13.append(np.array(
                        dataloader.dataset.gt[item[__]][1]['word_bbox'].copy(), dtype=np.int32))
        ...
        if np.array(all_count)[-min(100, len(all_count)):].sum() != 0:
            ...
            f_score = int(np.array(
                all_f_score)[-min(100, len(all_f_score)):].sum() * 10000 / count) / 10000
            precision = int(np.array(
                all_precision)[-min(100, len(all_precision)):].sum() * 10000 / count) / 10000
            recall = int(np.array(
                all_recall)[-min(100, len(all_recall)):].sum() * 10000 / count) / 10000
        else:
        ...
        for no, (image, image_name, original_dim, item) in enumerate(iterator):
            ...
            for i in range(output.shape[0]):
                ...
                score_calc = calculate_fscore(
                    resize_bbox(original_dim[i], output[i], config)[
                        'word_bbox'][:, :, 0, :],
            ...
            iterator.set_description(
                'F-score: ' + str(np.mean(all_accuracy)) +
                '| Cumulative F-score: '
